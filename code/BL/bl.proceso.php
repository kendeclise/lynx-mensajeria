    ...
    {   $_aLeyenda= $objClaseD->fcListarLeyenda(0);
        ...
        if($action == "procesar")
        {
            ...
            if (trim($_idDocumentoSeleccionado)!="0" && trim($_idDatoOpcion)!="0")
            {
                ...
                $referencia = fcExtraerValor($_aLeyenda , "[REFERENCIA]", $_documentoCompleto,$_pefilUsu); 
                ...
                if (sizeof($_aCorreo)>0)
                {
                    for ($_cr=0; $_cr<sizeof($_aCorreo); $_cr++)
                    {
                        ...
                        if ($_enviaCorreoE=='S')
                        {
                                    ...
                                    $mail->Subject = $_documentoCompleto."-".(($_codIncidencia<>"")?$_descripcionIncid." - Ref. ".$referencia:$_tituloCorreoMSG); //(($_codIncidencia<>"")?" - INCIDENCIA ":"");
                                    $mail->Body    = ($_cuerpo) ;
                                    if (!$mail->Send()) 
                                    {  echo  $mail->ErrorInfo; } 
